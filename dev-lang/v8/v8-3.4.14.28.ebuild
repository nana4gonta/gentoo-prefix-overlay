...
# $Header: /var/cvsroot/gentoo-x86/dev-lang/v8/v8-3.4.14.28.ebuild,v 1.3
# 2011/10/17 18:31:01 nativemad Exp $
...
src_prepare() {
	...
	# Respect the user's CFLAGS, including the
	# optimization level.
	epatch "${FILESDIR}"/${PN}-no-O3-r0.patch
}

src_configure() {
	# GCC issues multiple warnings about
	# strict-aliasing issues in v8 code.
...
src_compile() {
	...
	# Use target arch detection
	# logic from bug #354601.
	case ${CHOST} in
		i?86-*)
			myarch=x86
			;;
